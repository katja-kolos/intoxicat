...
class LSTM_Model(nn.Module):
    ...
    def __init__(self, input_size, layers_sizes, num_layers, num_classes, dropout, bidirectional, bias):
    ...
    def forward(self, input_data, input_lengths, dropout, bn):
        ...
        # put output from LSTM through linear sigmoid activation function and put the result trhough another linear layer
        ...
        output_norm = first_batch_norm(output)

        output_sigmoid = self.sigmoid(output_norm)
        # print(f'Output after sigmoid: {output_sigmoid}')

        # go through all of the linear layers
        input_linear_sigmoid = output_sigmoid
